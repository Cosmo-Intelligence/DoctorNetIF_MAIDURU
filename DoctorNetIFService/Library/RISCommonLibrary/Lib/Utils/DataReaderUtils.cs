...
namespace RISCommonLibrary.Lib.Utils
{
	public static class DataReaderUtils
	{
		...
		public static string GetStringByDBValue(this IDataReader dr, string fieldName)
		{
			return GetStringByDBValue(dr, fieldName, string.Empty);
		}

		public static string GetStringByDBValue(this IDataReader dr, string fieldName, string defaultValue)
		{
			int idx = dr.GetOrdinal(fieldName);
			if (dr.IsDBNull(idx))
			{
			...
			return dr.GetValue(idx).ToString();
		}
	}
}
