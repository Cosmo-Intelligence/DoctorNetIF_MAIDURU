...
{
	public class PATIENTINFOUpdaterChangeWard : BaseUpdater
	{
		#region const
		...
		public override void SetParams(BaseMsgData data, System.Data.IDbCommand command)
		{
			...
			ChangeWardMsgData targetData = (ChangeWardMsgData)data;
			ChangeWardAggregate root = targetData.Request.MsgBody.ChangeWard;

			SetStringToCommand(PARAM_NAME_KANJA_ID, root.PT_ID.TrimData, command);

			SetStringToCommand(PARAM_NAME_BYOUTOU_ID, root.IDO_BYOTO_CD.TrimData, command);
			SetStringToCommand(PARAM_NAME_BYOUSITU_ID, root.IDO_ROOM_CD.TrimData, command);
		}
	}
}
