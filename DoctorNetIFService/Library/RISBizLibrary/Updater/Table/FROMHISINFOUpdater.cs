...
namespace RISBizLibrary.Updater.Table
{
	public abstract class FROMHISINFOUpdater : BaseUpdater
	{
		...
		public override void SetParams(BaseMsgData data, System.Data.IDbCommand command)
		{
			...
			if (MessageLogger.Instance.Enc.GetByteCount(data.Request.TextMessage) > 4000)
			{
				SetStringToCommand(PARAM_NAME_RECIEVETEXT, string.Empty, command); //受信電文
			}
			else
			{
		...
		public void Delete(BaseMsgData data, IDbCommand command)
		{
			...
			string DELETE_SQL =
				"DELETE FROM fromhisinfo " +
				"WHERE " +
					"RECIEVEDATE < (sysdate - :keepday) " +
				"AND " +
					"MESSAGETYPE in (" + state.GetDeleteMESSAGETYPE() +") " +
				" ";
