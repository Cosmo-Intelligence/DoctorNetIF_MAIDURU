...
namespace RISBizLibrary.Upload
{
	internal static class ToHisInfoHelper
	{
		...
		public static void SetDataFromResponseMsg(ToHisInfo target, BaseMsgData msgData)
		{
			...
			target.TransferResult = RQRISDBConst.TOHISINFO_TRANSFERRESULT_NORMAL;
		...
		public static void Delete(IDbConnection cn)
		{
			using (IDbCommand command = cn.CreateCommand())
			{
				command.InitCommandODP(ConfigurationManager.AppSettings["CommandTimeout"].StringToInt32());
				...
				MiscUtils.WriteDbCommandLogForLog4net(command, _log);
				_log.Debug("TOHISINFOテーブル削除します");

				command.ExecuteNonQuery();

				_log.Debug("TOHISINFOテーブル削除しました");
			}
		}
		...
		public static void InsertToSynapseInfo(ToHisInfo toHisInfo, IDbConnection cn)
		{
			// 実施以外は登録しない
			if (toHisInfo.RequestType != RQRISDBConst.REQUEST_TYPE_EXAM)
			{
				return;
			}
			...
			using (IDbCommand command = cn.CreateCommand())
			{
				...
				#region パラメータ設定
				const string PARAM_REQUESTTYPE = "REQUESTTYPE";
				const string PARAM_REQUESTUSER = "REQUESTUSER";
				const string PARAM_REQUESTTERMINALID = "REQUESTTERMINALID";
				const string PARAM_MESSAGEORDERID = "MESSAGEORDERID";
				const string PARAM_MESSAGEPATIENTID = "MESSAGEPATIENTID";

				command.Parameters.Clear();
				IDataParameter param = command.CreateParameter();
				param.SetInputString(PARAM_REQUESTTYPE, "OI01");
				command.Parameters.Add(param);
				param = command.CreateParameter();
				param.SetInputString(PARAM_REQUESTUSER, "RIS_UP");
				command.Parameters.Add(param);
				param = command.CreateParameter();
				param.SetInputString(PARAM_REQUESTTERMINALID, "RISSERVER");
				command.Parameters.Add(param);
				param = command.CreateParameter();
				param.SetInputString(PARAM_MESSAGEORDERID, toHisInfo.MessageID1);
				command.Parameters.Add(param);
				param = command.CreateParameter();
				param.SetInputString(PARAM_MESSAGEPATIENTID, toHisInfo.MessageID2);
				command.Parameters.Add(param);
				#endregion

				MiscUtils.WriteDbCommandLogForLog4net(command, _log);
				_log.Debug("TOSYNAPSEINFOテーブル登録します");

				command.ExecuteNonQuery();

				_log.Debug("TOSYNAPSEINFOテーブル登録しました");
			}
		}
