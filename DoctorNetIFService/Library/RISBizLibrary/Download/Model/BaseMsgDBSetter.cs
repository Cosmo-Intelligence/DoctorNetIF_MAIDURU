...
namespace RISBizLibrary.Download.Model
{
	public abstract class BaseMsgDBSetter
	{
		...
		/// <returns></returns>
		public bool SetDataToDatabase(BaseMsg msg)
		{
			if (msg == null)
			{
			...
			_log.DebugFormat("接続文字列取得しました。{0}",
					ODPConnectionUtils.GetConnectionStringWithoutPassword(connectionStringRis));

			_log.Debug("DB接続します");
			IDbConnection cnRis = ODPConnectionUtils.Connect(connectionStringRis);
			_log.Debug("DB接続しました");

			try
			{
				return SetDataToDatabaseInner(_msgData, cnRis);
			}
			finally
			{
				cnRis.Close();
		...
		/// <param name="cn"></param>
		/// <returns></returns>
		protected abstract bool SetDataToDatabaseInner(BaseMsgData msgData, IDbConnection cnRis);
